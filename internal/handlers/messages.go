...
func (h *MessageHandler) UpsertDeviceKey(c *gin.Context) {
	...
	_, err := h.db.Exec(`
		INSERT INTO user_device_keys (user_id, device_id, algorithm, public_key, key_id)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT(user_id, device_id, key_id)
		DO UPDATE SET algorithm = excluded.algorithm, public_key = excluded.public_key, revoked_at = NULL
